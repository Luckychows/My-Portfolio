  ...
  return (
    <section id="about" className="px-5 py-24 sm:px-8">
      <div className="mx-auto max-w-7xl">
        <Reveal>
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-red-200">About</p>
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[1.4fr_.8fr]">
                ...
                <div className="grid gap-3">
                  {resume.certifications.map((cert) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="group flex items-center justify-between gap-3 rounded-md border border-red-300/15 bg-red-300/[0.08] px-4 py-3 text-sm text-red-50 transition hover:border-red-200/55 hover:bg-red-300/15"
                    >
                      <span>{cert.name}</span>
                      <ExternalLink className="size-4 shrink-0 text-red-200 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                    </a>
                  ))}
                </div>
              </div>
          ...
          <Reveal delay={0.1}>
            <SpotlightCard className="h-full">
              ...
              <p className="mt-2 text-red-200">{resume.education.period}</p>
              <div className="mt-8">
